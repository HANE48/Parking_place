...
function ParkingPriceTable(props) {
    ...
    const list = data.map((item) => {
        ...
        let bscPrice = item.bsc_prk_crg + '원' + (item.bsc_prk_hr + '분')
        // 추가요금:ADD_PRK_CRG, 시간분단위:ADD_PRK_HR 
        let addPrice = item.add_prk_crg + '원' + (item.add_prk_hr + '분')
        //월정기요금:PRD_AMT
        let prdPrice = item.prd_amt + '원'
        ...
        return (
            ...
            <tr>
                ...
                {/* // 수정/삭제 버튼 */}
                <td>
                    <button className="edit_btn" onClick={()=>(alert(item.pklt_nm + '수정창을 엽니다'))} >수정</button>
                </td> 
            </tr>
            )
})
